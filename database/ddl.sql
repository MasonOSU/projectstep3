...
CREATE TABLE `Citations` (
  ...
  CONSTRAINT `citing_paper_id` FOREIGN KEY (`citing_paper_id`) 
    REFERENCES `Research_Papers` (`research_paper_id`) 
    ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `cited_paper_id` FOREIGN KEY (`cited_paper_id`) 
    REFERENCES `Research_Papers` (`research_paper_id`) 
    ON DELETE CASCADE ON UPDATE NO ACTION);
...
INSERT INTO `Authors` (`first_name`, `last_name`)
  VALUES ('Nina', 'Faulkner'), ('Robert', 'Ball'), ('John', 'Smith'),
          ('Sandra', 'Smith'), ('Maria', 'Thompson');
...
INSERT INTO `Disciplines` (`field`)
  VALUES ('Biology'), ('Literature'), ('History'), ('Psychology');

-- -- Insert `Research_Papers` data.
INSERT INTO `Research_Papers` (`title`, `date_published`, `doi`, `institution_id`, `discipline_id`)
  VALUES 
    ('Grapefruit Juice Interactions with Antipsychotic Medicine', '1980-10-29', '10.1991/1123',
      (SELECT `institution_id` FROM `Institutions` WHERE `name` = 'Weston Biological Institute'),
      (SELECT `discipline_id` FROM `Disciplines` WHERE `field` = 'Biology')),
    ('Celtic Tradition in Beowulf', '1999-01-03', '10.9932/1939',
      (SELECT `institution_id` FROM `Institutions` WHERE `name` = 'Smith and Harper College'),
      (SELECT `discipline_id` FROM `Disciplines` WHERE `field` = 'Literature')),
    ('Schizophrenia Treatment Models', '1997-04-09', '10.0001/9634',
      (SELECT `institution_id` FROM `Institutions` WHERE `name` = 'Samoa University'),
      (SELECT `discipline_id` FROM `Disciplines` WHERE `field` = 'Psychology')),
    ('An Overview of Antipsychotic Drug Kinetics', '1978-05-13', '10.0193/9294', NULL,
      (SELECT `discipline_id` FROM `Disciplines` WHERE `field` = 'Biology')),
    ('Anglo-Saxon Class Dynamics and Migration', '1991-07-21', '10.1111/2345',
      (SELECT `institution_id` FROM `Institutions` WHERE `name` = 'Smith and Harper College'),
      (SELECT `discipline_id` FROM `Disciplines` WHERE `field` = 'History'));
...
INSERT INTO `Citations` (`citing_paper_id`, `cited_paper_id`)
  VALUES 
    ...
    (SELECT `research_paper_id` FROM `Research_Papers` 
      WHERE `title` = 'An Overview of Antipsychotic Drug Kinetics')),
    ((SELECT `research_paper_id` FROM `Research_Papers` 
      WHERE `title` = 'Celtic Tradition in Beowulf'),
    (SELECT `research_paper_id` FROM `Research_Papers`
      WHERE `title` = 'Anglo-Saxon Class Dynamics and Migration')),
    ((SELECT `research_paper_id` FROM `Research_Papers` 
      WHERE `title` = 'Schizophrenia Treatment Models'),
    (SELECT `research_paper_id`FROM `Research_Papers`
      WHERE `title` = 'An Overview of Antipsychotic Drug Kinetics')),
    ((SELECT `research_paper_id` FROM `Research_Papers`
      WHERE `title` = 'Celtic Tradition in Beowulf'),
...
INSERT INTO `Research_Papers_has_Authors` (`paper_id`, `researcher_id`)
  VALUES 
    ...
    (SELECT `author_id` FROM `Authors` 
      WHERE `first_name` = 'Nina' AND `last_name` = 'Faulkner')),
    ((SELECT `research_paper_id` FROM `Research_Papers` 
      WHERE `title` = 'Grapefruit Juice Interactions with Antipsychotic Medicine'),
    (SELECT `author_id` FROM `Authors` 
      WHERE `first_name` = 'Robert' AND `last_name` = 'Ball')),
    ((SELECT `research_paper_id` FROM `Research_Papers` 
      WHERE `title` = 'Celtic Tradition in Beowulf'),
    (SELECT `author_id` FROM `Authors` 
      WHERE `first_name` = 'Nina' AND `last_name` = 'Faulkner')),
    ...
    ((SELECT `research_paper_id` FROM `Research_Papers`
      WHERE `title` = 'An Overview of Antipsychotic Drug Kinetics'),
    (SELECT `author_id` FROM `Authors`
      WHERE `first_name` = 'Sandra' AND `last_name` = 'Smith')),
    ((SELECT `research_paper_id` FROM `Research_Papers`
      WHERE `title` = 'Anglo-Saxon Class Dynamics and Migration'),
    (SELECT `author_id` FROM `Authors`
      WHERE `first_name` = 'Nina' AND `last_name` = 'Faulkner'));
