...
let updateResearchPapersHasAuthorsForm = document.getElementById("update-research_papers_has_authors-form-ajax");
...
updateResearchPapersHasAuthorsForm.addEventListener("submit", function (e) {
	...
	let inputResearchPaperAuthor = document.getElementById("input-research_paper_author_id-update");
...
function updateRow(data, researchPaperAuthorId) {
	...
	for (let dataIndex = 0; dataIndex < parsedData.length; dataIndex++) {
		if (parsedData[dataIndex].research_paper_id == researchPaperAuthorId) {
			...
			for (let parsedIndex = 0, row; (row = table.rows[parsedIndex]); parsedIndex++) {
				if (table.rows[parsedIndex].getAttribute("data-value") == researchPaperAuthorId) {
